...
def quantize_linear_layer(quantizer, weight_tensor, layer_name):
    ...
    if weight_tensor.dim() != 2:
        raise ValueError(f"Expected 2D weight tensor, got {weight_tensor.dim()}D for {layer_name}")
    
    # Check if dimensions are compatible
    M, N = weight_tensor.shape
    if N % 16 != 0:
        raise ValueError(f"N dimension ({N}) must be multiple of 16 for {layer_name}")
...
def quantize_model_checkpoint(
    ...
    if os.path.isdir(input_dir):
        ...
        safetensors_files = [Path(input_dir)] if input_dir.endswith('.safetensors') else []
    ...
    for safetensors_file in safetensors_files:
        ...
        with safe_open(safetensors_file, framework="pt") as f:
            ...
            for key in tqdm(keys, desc=f"  Quantizing {safetensors_file.name}"):
                has_valid_key = any(valid_key in key for valid_key in valid_keys)
                has_invalid_key = any(invalid_key in key for invalid_key in invalid_keys)
